/* ============================================================
   ...
   Portado de docs/design/punku-symbol.jsx. Variantes: mark / hero / open.
   El "momento mágico" (A4) abre la hoja de la puerta y revela luz dorada.
   Respeta prefers-reduced-motion (ver .mtn-leaf en globals.css).
   ============================================================ */
"use client";
import { useId } from "react";

export function MountainDoor({
  size = 120,
  variant = "mark",
  open = false,
  className = "",
  ...
  size?: number;
  variant?: "mark" | "hero" | "open";
  open?: boolean;
  className?: string;
}) {
  const raw = useId();
  const uid = "md" + raw.replace(/[:]/g, "");
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="PUNKU — la montaña que abre su puerta"
    >
      <defs>
      ...
      <circle className="mtn-spill" cx="74" cy="80" r="30" fill={`url(#${uid}-glow)`} opacity={isOpen ? 1 : 0} />

      {/* Marco de la puerta */}
      <path d="M60 100 L60 74 Q60 62 74 62 Q88 62 88 74 L88 100 Z" fill="#23201A" opacity="0.18" />
      <path d="M62 100 L62 75 Q62 64 74 64 Q86 64 86 75 L86 100 Z" fill={`url(#${uid}-gold)`} />
      <path d="M65 100 L65 76 Q65 67 74 67 Q83 67 83 76 L83 100 Z" fill="#FFF3D2" opacity={isOpen ? 0.95 : 0.7} />
