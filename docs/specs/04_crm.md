...
- **ESTADO DE IMPLEMENTACIÓN — Módulo de co-construcción por WhatsApp en B4 (construido):** card full-width dentro de B4, **abierta por defecto** cuando el expediente llegó con poca información (`datos_incompletos`). Tres pasos: (1) **"Generar preguntas"** → Claude Haiku arma preguntas dinámicas y específicas del caso con opciones numeradas (banco estático como fallback); (2) **"Contactar por WhatsApp"** → abre `wa.me` con el teléfono real (resuelto server-side) y las preguntas pre-redactadas; (3) pegar la respuesta del ciudadano ("1 2 1 2") + **"Interpretar"** → un parser determinista compone el texto formal y rellena los campos ámbar (objetivo, metas, recursos, metodología), subiendo el progreso. **Filtrado (construido):** el generador **solo pregunta lo genuinamente pendiente** — nunca la meta/plazo/beneficiarios que el ciudadano ya eligió con botones ni los campos ya llenos en B4; si no falta nada, muestra "No hay nada pendiente que preguntar". Lleva rótulo honesto (demo: interpretación manual funcional; producción: WhatsApp Business API automática). Ver spec 03 §6.quater para el motor. Verificado en producción: las preguntas son de Haiku (dinámicas, no del banco) y el filtrado omite lo ya resuelto.
...
- [x] Módulo de co-construcción WhatsApp en B4: genera preguntas (Haiku) → wa.me server-side → interpreta la respuesta y rellena campos.
- [x] El filtrado de co-construcción no repregunta lo elegido con botones ni lo ya lleno; indica cuándo no falta nada.
