...
- [ ] Una sola llamada produce el JSON con los 5 campos (categoría, modalidad, urgencia, facultades, resumen).
- [ ] A la IA nunca se le envían datos personales.
- [ ] Si la IA falla, el fallback por reglas genera el expediente igual.
- [ ] El JSON se valida contra esquema antes de usarse.
- [ ] El motor está aislado tras una interfaz (migrable sin reescribir el sistema).
- [ ] La spec documenta la migración Claude → DeepSeek/Huawei para producción.
